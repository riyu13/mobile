...
const KataBendaEdit = () => {
  ...
  const onSubmitForm = async () => {
    try {
      await axios.patch(`http://192.168.1.5:5000/katabenda/${params.uuid}`, form);
  ...
  const getKataBenda = async () => {
    try {
      const response = await axios.get(`http://192.168.1.5:5000/katabenda/${params.uuid}`);
  ...
  return (
    <View style={styles.container}>
      <Text style={{ fontSize: 20, marginBottom: 20 }}>Edit Kata Benda</Text>
      <ScrollView>
        <TextInput
          label="Kanji"
          value={form.kanji}
