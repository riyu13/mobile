...
const TabelKataBenda = () => {
  ...
  const editData = uuid => {
    ...
    navigation.navigate('KataBenda Edit', { uuid });
  };
  ...
  Alert.alert(
    ...
    [
      ...
      { 
        text: 'OK', 
        onPress: () => {
          axios.delete(`http://192.168.1.5:5000/katabenda/${KataBendaId}`)
            .then(response => {
 ...
 const getKataBenda = async() => {
        const response = await axios.get(`http://192.168.1.5:5000/katabenda?search_query=${keyword}&page=${page}&limit=${limit}`);
  ...
  const filteredData = katabenda.filter(item =>
  ...
  return (
    ...
    <Text style={{fontSize: 20}}>Daftar Kata Benda N5</Text>
      <Searchbar
        placeholder="Search"
      ...
      <DataTable>
        ...
        {pageData.map((item, index) => (
          <DataTable.Row key={item.id} style={{ backgroundColor: index % 2 === 0 ? '#FFFFE0' : '#F0E68C', borderRadius: 5, marginBottom: 1 }}>
            ...
            <DataTable.Cell style={{width: 80}}><Button onPress={()=>deleteKataBenda(item.uuid)}><FontAwesome5 name="trash" size={18} color="#8B4513"/></Button></DataTable.Cell>
          </DataTable.Row>
        ))}
