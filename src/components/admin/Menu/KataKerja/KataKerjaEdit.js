...
const KataKerjaEdit = () => {
  ...
  const onSubmitForm = async () => {
    try {
      await axios.patch(`http://192.168.1.5:5000/katakerja/${params.uuid}`, form);
      Alert.alert('Berhasil', 'Data kata kerja berhasil diupdate', [
        {
          text: 'OK',
  ...
  const getKataKerja = async () => {
    try {
      const response = await axios.get(`http://192.168.1.5:5000/katakerja/${params.uuid}`);
  ...
  return (
    <View style={styles.container}>
      <Text style={{ fontSize: 20, marginBottom: 20 }}>Edit Kata Kerja</Text>
      <ScrollView>
        <TextInput
          label="Kanji"
          value={form.kanji}
